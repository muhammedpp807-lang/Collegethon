...
import { COURSES, type ClassGroup, type Student } from "../data";
...
const blank = (classId = "class-first"): Student => ({
  id: "",
  name: "",
  rollNo: "",
  course: COURSES[0],
...
export default function StudentModal({ open, initial, classes, onClose, onSave }: Props) {
  const [form, setForm] = useState<Student>(blank(classes[0]?.id));
  const [error, setError] = useState("");

  useEffect(() => {
    if (open) {
      setForm(initial ? { ...initial } : blank(classes[0]?.id));
      setError("");
    }
        ...
        <div className="relative mt-6 grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            ...
            <textarea
              ...
              placeholder="Short student profile note"
            />
          </div>
          <div>
