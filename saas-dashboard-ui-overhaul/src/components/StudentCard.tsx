...
export default function StudentCard({ student, index, className, onView, onEdit, onDelete }: Props) {
    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div
            className={`flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-br ${grad} text-base font-bold text-white shadow-lg`}
          >
            {student.photo ? (
      ...
      <div className="relative mt-4 grid grid-cols-1 gap-2">
        <div className="glass-soft flex items-center gap-2 rounded-xl px-3 py-2 text-xs text-white/70">
          ...
          {className && <span className="ml-auto rounded-full bg-white/10 px-2 py-0.5 text-[10px] text-white/55">{className}</span>}
        </div>
      ...
      <div className="relative mt-5 flex gap-2">
        <button
          ...
          className="flex items-center justify-center gap-2 rounded-xl bg-white/5 px-3 py-2.5 text-xs font-semibold text-cyan-300 ring-1 ring-white/10 transition-all duration-300 hover:-translate-y-0.5 hover:bg-cyan-500/20 hover:text-white hover:shadow-lg hover:shadow-cyan-500/30 hover:ring-cyan-400/50"
        >
