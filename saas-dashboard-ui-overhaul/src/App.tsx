...
const STUDENT_PASSWORDS_KEY = "edupulse.studentPasswords";
...
// Generate default password for students (rollNo + "2024")
function genStudentPassword(rollNo: string): string {
  return `${rollNo.toLowerCase()}2024`;
}
...
export default function App() {
  ...
  const [studentPasswords, setStudentPasswords] = useState<Record<string, string>>(() =>
    loadJSON(STUDENT_PASSWORDS_KEY, {})
  );
  ...
  useEffect(() => {
    localStorage.setItem(STUDENT_PASSWORDS_KEY, JSON.stringify(studentPasswords));
  }, [studentPasswords]);
  ...
  const handleLogin = (u: string, p: string, mode: LoginMode) => {
    // Admin login
    if (mode === "admin") {
    ...
    // Student login - rollNo as username
    const student = students.find((s) => s.rollNo.toLowerCase() === u.toLowerCase());
    if (student) {
      const storedPass = studentPasswords[student.id] || genStudentPassword(student.rollNo);
      if (p === storedPass) {
        setAuthUser(student.rollNo.toLowerCase());
  ...
  // Check if logged in user is a student (by roll number)
  const loggedStudent = students.find(
    (s) => s.rollNo.toLowerCase() === authUser.toLowerCase()
  );
  const isAdmin = authUser === creds.username;
  ...
  if (loggedStudent && !isAdmin) {
    return (
      <StudentPortal
        ...
        onPasswordChange={(id: string, pass: string) => {
          setStudentPasswords((prev) => ({ ...prev, [id]: pass }));
