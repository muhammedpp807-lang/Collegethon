...
type Props = {
  onLogin: (username: string, password: string, mode: LoginMode) => boolean;
};
...
export default function Login({ onLogin }: Props) {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    const ok = onLogin(username.trim(), password, mode);
    if (!ok) {
  ...
  return (
    <div className="app-bg flex min-h-screen items-center justify-center p-4">
      ...
      >
        ...
        <div className="relative mb-4 flex items-center justify-center gap-1 rounded-full bg-white/5 p-1 ring-1 ring-white/10">
          <button
            type="button"
        ...
        <form onSubmit={submit} className="relative mt-4 space-y-4">
          <div className="relative">
            ...
            <input
              className={field}
              placeholder={mode === "admin" ? "Username" : "Roll Number"}
              value={username}
        ...
        <div className="relative mt-6 rounded-xl bg-white/5 px-4 py-3 text-center text-xs text-white/40 ring-1 ring-white/10">
          {mode === "admin" ? (
            ...
            <>
              Student password = rollNo + "2024" (e.g., <span className="font-mono text-emerald-300">cs-20412024</span>)
            </>
          )}
        </div>
      </div>
